...
The [[https://gitlab.com/niklaseklund/consult-dtache][consult-dtache]] package provides the integration of =dtache= [[https://github.com/minad/consult][consult]]. The benefit of using this package is that the user has the ability to narrow the list of sessions based on different criterion. Since this package has external dependencies it is provided in its own repository.
